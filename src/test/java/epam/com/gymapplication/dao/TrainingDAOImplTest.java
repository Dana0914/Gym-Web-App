...
@ContextConfiguration(classes = {AppConfig.class, TrainingDAOImpl.class})
public class TrainingDAOImplTest {
    ...
    public void setUp() {
        ...
        trainingDAO.save(updatedTraining);
    ...
    public void save_withValidData_returnsValidEntity() {
        ...
        Training retrievedTrainer = trainingDAO.findById(4L).orElse(null);
    ...
    public void update_withExistingEntity_updatesEntityDetails() {
        Training existingTraining = trainingDAO.findById(5L).orElse(null);
        Assertions.assertNotNull(existingTraining);

        trainingDAO.update(existingTraining);

        Training retrievedTraining = trainingDAO.findById(5L).orElse(null);
    ...
    public void deleteById_withValidId_returnsValidEntity() {
        ...
        trainingDAO.deleteById(retrievedTraining.getId());

        Assertions.assertEquals(trainingDAO.findById(5L), Optional.empty());

    }

    @Test
    public void findById_withExistingId_returnsEntity() {
        Training retrievedTraining = trainingDAO.findById(4L).orElse(null);
        Assertions.assertNotNull(retrievedTraining);
    ...
    public void findAll_withExistingData_returnsValidEntities() {
        Set<Map.Entry<Long, Training>> all = trainingDAO.findAll();
    ...
    public void findByTrainingType_withExistingData_returnValidEntity() {
        Optional<Training> trainingType = trainingDAO.findByTrainingType(training.getTrainingType());
    ...
    public void findingByTrainingName_withExistingData_returnValidEntity() {
        Optional<Training> trainingName = trainingDAO.findByTrainingName(training.getTrainingName());
